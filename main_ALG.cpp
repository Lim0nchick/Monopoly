...
int main()
{
	...
	for (auto i=0; i<9; i++)
	{
		...
		//r1[i]=pool.runAsync<unsigned>(&try1,(i+3), 0,0,(n-1),field, lChance);

		f1[i] = async(try1,(i+3), 0,0,(n-1),field, lChance);
		cout << r1[i] << "\t";
		if (max < r1[i])
		{
			max = r1[i];
 	...
 	for (unsigned short d1=2; d1<14; d1+=2)
 	{
 		...
 		for (unsigned short d2 = 0; d2<9; d2++)
 		{
 			if ((d1 == 4) || (d1 == 6) || (d1 == 8) || (d1 == 10))
				r2[d1][d2] = pool.runAsync<unsigned>(&try2,d1, (d2+3),(n-1), (r1[d1-3]), field, ((r1[d1-3])%n), lChance);
			else 
				r2[d1][d2] = pool.runAsync<unsigned>(&try2,d1, (d2+3),(n-1), 0, field, 0, lChance);
 			if (max < r2[d1][d2])
 			{
 				max=r2[d1][d2];
 	...
 	for (unsigned short d3 = 3; d3<12; d3++)
 	{
 		...
 		for (unsigned short d2 = 2; d2 < 76; d2+=2)
 		{
 			...
 			if ((d2%12) == 4 || (d2%12) == 6 || (d2%12) == 8 || (d2%12) == 10)
 			{
				...
				r3[d3][d2] = pool.runAsync<unsigned>(&try3,d1, (d2%12),d3, (n-1), (r2[d1][d2%12]), CurPos, field, lChance);
				//cout << "r3[" << d3 << "][" << d2 << "]" << endl;
 			}
			else
			{
				...
				//cout << "CurPos = " << CurPos << endl;
 				if (CurPos == 20) CurPos = 0;
				r3[d3][d2] = pool.runAsync<unsigned>(&try3,d1, (d2%12),d3, (n-1), 0, 0, field, lChance);
				//cout << "r3[" << d3 << "][" << d2 << "]" << endl;
			}
			if (max < r3[d3][d2])
			{
